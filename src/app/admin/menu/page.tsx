    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="flex overflow-x-auto pb-4 gap-3 lg:flex-col lg:overflow-visible lg:space-y-2 lg:gap-0 hide-scrollbar">
            {catLoading ? (
              <div className="flex justify-center p-4"><Loader2 className="animate-spin text-[#D4A24C]" /></div>
            ) : categories.map((cat) => (
              <div
                key={cat.id}
                className="flex items-center justify-between min-w-[160px] lg:min-w-0 rounded-xl border border-[#D4A24C]/20 bg-linear-to-br from-[#1A1410] to-[#0D0B09] px-4 py-3 text-sm font-medium text-[#F7F3EC] hover:border-[#D4A24C] hover:shadow-[0_0_15px_rgba(212,162,76,0.15)] transition-all cursor-pointer group shrink-0"
              >
                <span className="truncate pr-2">{cat.name}</span>
                <button 
                  onClick={() => handleOpenCategoryModal(cat)}
                  className="p-1.5 rounded bg-black/50 text-[#C7BFB2] hover:text-[#D4A24C] transition-colors lg:opacity-0 lg:group-hover:opacity-100 shrink-0"
                >
                  <Edit2 size={14} />
                </button>
              </div>
