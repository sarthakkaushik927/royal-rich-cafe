...
export function ChefMobileBottomNav() {
  ...
  const navItems = [
    { label: 'Kitchen', path: '/chef', icon: ChefHat },
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 flex justify-center z-[100] pb-0">
      <div className="relative bg-[#FFB846] w-full h-[62px] rounded-t-[32px] flex items-center w-full shadow-[0_-10px_40px_rgba(255,184,70,0.15)]">
        ...
        {activeIndex !== -1 && (
          <div 
            className="absolute top-0 left-0 h-[56px] w-[33.333%] transition-transform duration-500 ease-in-out flex justify-center pointer-events-none"
            style={{ transform: `translateX(${activeIndex * 100}%)` }}
          >
            {/* SVG for smooth cutout - dips down to create a black background for the icon */}
            <svg width="120" height="46" viewBox="0 0 86 46" className="fill-[#0D0B09] -mt-[1px]">
        ...
        {navItems.map((item, index) => {
          ...
          return (
            <Link 
              key={item.path} 
              href={item.path} 
              className="relative flex flex-col items-center justify-center w-[33.333%] h-full z-10 tap-highlight-transparent pt-1"
            >
